...
function backend(cmdStr) { // send this to Kotlin (or simulate in case of browser-only testing)
  ...
  if (cmdStr[0] == 'ready')
    ...
    b2f_new_event(e)
  } else if(cmdStr[0] == 'priv:board') {
    var operation = JSON.parse(atob(cmdStr[1]))
    ...
    var e = { 'header': {'tst': Date.now(),
                          ...
                          'fid': myId,
                          'seq': tremola.seq},
                  'confid': {'type': 'board', 'operation': operation, 'recps': cmdStr},
                  'public': {}
    }
    console.log('e=', JSON.stringify(e))
    b2f_new_event(e)
...
function resetTremola() { // wipes browser-side content
    ...
    if (typeof Android == 'undefined') //simulates seq for browser only testing
      tremola.seq = 0
    var n = recps2nm([myId])
...
function persist() {
    console.log('persisting', tremola);
    var k = "tremola-" + myId;
    window.localStorage.setItem(k, JSON.stringify(tremola));
}
...
function b2f_new_event(e) { // incoming SSB log event: we get map with three entries
  ...
  if (e.confid && e.confid.type == 'post') {
  ...
  } else if (e.confid && e.confid.type == 'board') {
    var i;
    var bid;
    for (i in e.confid.recps) {
      var r = e.confid.recps[i];
      if (!(r in tremola.contacts)) {
        var a = id2b32(r);
        tremola.contacts[r] = { "alias": a, "initial": a.substring(0,1).toUpperCase(),
                                "color": colors[Math.floor(colors.length * Math.random())] }
    ...
    if (!(e.header.ref in board.operations)) { //new operation
      if(e.confid.operation.cmd[0] == Operation.BOARD_CREATE) {
        board.name = e.confid.operation.cmd[1]
      }
...
function b2f_new_contact(contact) { // [fid, {"alias": "nickname", "initial": "A", "color" ...}
    // var c = JSON.parse(contact_str)
    if (contact[0] != myId)
        tremola.contacts[contact[0]] = contact[1];
    console.log(tremola);
    load_contact_item(contact)
}
...
function b2f_initialize(id) {
    ...
    if (k in window.localStorage) {
        tremola = JSON.parse(window.localStorage.getItem(k));
